...
{
    write_log("[%s] received UNKNOWN on '%s' from %s", server_name, target,
              client_ip);
    write_log("[%s] responding with %d to %s for UNKNOWN on '%s'", server_name,
              status_code, client_ip, target);
}
