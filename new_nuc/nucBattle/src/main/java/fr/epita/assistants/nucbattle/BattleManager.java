...
public class BattleManager {
    ...
    public void computeBattle(String reportPath) throws IOException {
	    ...
	    if (rootNode == null || rootNode.get("NUCs") == null || rootNode.get("battle") == null) {
	    ...
	    if (battle == null || battle.getBattleId() < 0) {
                report = new Report();
	...
	Optional<String> beforeWinner = determWinner(nucsMap);
    	if (beforeWinner.isPresent()) {
        	Report report = new Report();
        	report.setType(ReportType.WINNER);
        	report.setPlayer(beforeWinner.get());
        	report.setOutcome(computeFinalStatus(nucsMap));
        	return report;
    }
