...
static void add_command(struct minimake_context *c, const char *cmd)
{
    if (c->rule_count == 0)
    {
	    printf("DEBUG add_command: no current rule!\n");
        return;
    }

    struct rule *rule = c->rules[c->rule_count - 1];

     printf("DEBUG add_command: before - rule '%s' has %d commands\n", rule->target, rule->cmd_count);
    ...
    printf("DEBUG add_command: adding command '%s'\n", cmd); // Ajoute ce debug
    if (c->pretty_print)
    {
	...
	printf("DEBUG add_command: expanded command '%s'\n", expanded_cmd);


        rule->commands[rule->cmd_count - 1] = mystrdup(expanded_cmd);
        free(expanded_cmd);
    }
    printf("DEBUG add_command: after - rule '%s' now has %d commands\n", rule->target, rule->cmd_count);
}
...
int handle_rule_line(char *line, struct minimake_context *c, int *in_recipe)
{
    ...
    *colon = '\0';
	...
	char *saveptr = NULL;
        ...
        while (dep)
        {
	    char *trimmed_dep = mystrdup(dep);
            trimmed_dep = trim_whitespace(dep);
            if (*trimmed_dep != '\0')
            {
                char *expanded_dep = substitute_variables_recursive(trimmed_dep, c);
...
int handle_command_line(char *line, struct minimake_context *c, int *in_recipe)
{
    if (!*in_recipe)
    {
	printf("DEBUG: not in recipe, ignoring line\n");
        return 0;
    }
    ...
    printf("DEBUG: after spaces, char=%d\\n", (int)*cmd);
...
int read_makefile(const char *filename, struct minimake_context *c)
{
    ...
    while ((read = getline(&line, &len, file)) != -1)
    {
        ...
        case LINE_RULE:
            ...
            break;
        case LINE_COMMAND:
            if(handle_command_line(line, c, &in_recipe))
		    in_recipe = 1;
	    else
		    in_recipe = 0;
            break;
